...
- overlay 的 visual style 不再自己从官方 app bundle 裁 `SoftwareCursor` 小图；主 MCP runtime 现在在 `OpenComputerUseKit` 内部复刻 `CursorMotion` procedural fallback 的 pointer/fog glyph 校准，命中点 anchor 固定在 `126x126` 画布里的同一组 tip-offset 上。runtime motion 状态继续按截图 / CGWindow 的 y-down 坐标计算，进入 AppKit 绘制前才做角度和 `dy` 翻转，保持方向语义与 `CursorMotion` 一致，但不让实验线依赖 runtime 代码。
